...
public class TikaCeremony extends FestivalActivity{
    ...
    public void planActivity() throws FestivalPlanningException, InvalidGuestException, BudgetExceededException{
        if(expectedGuests < 5){
            throw new InvalidGuestException("Where is everyone? There are not enough to perform Tika Ceremony! Minimum 5 guests required.");
        }
        if(estimatedCost > 50000){
            throw new BudgetExceededException("Why are there so many people? The cost will be to high");
        }
    }
